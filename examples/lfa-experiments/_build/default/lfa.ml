...
(* check this again, check if this works for large LoC *)
(* let var_num = !range_num/5 
let var_basic = !range_basic/5  *)

(* treba 5 ovde *)
(* let var_div = 5  *)
let var_div = if (!range_num > 10000) then 3 else 5 
let repeats_main = if (!range_num > 10000) then 4 else 4

let var_num = !range_num / var_div
(* let var_num_lower = var_num / 2 *)
(* let var_num_lower = var_num / 2 *)
(* let var_num_lower = var_num*2  *)

(* OVO JOS PROVERI *)
let var_num_lower = var_num / 2
let var_basic = !range_basic / var_div 
  ...
  while not !quit_loop do
    if !safe_counter > safe_max then
      ...
      let accept = if debug then 
        ...
        && num_lines > range_num1 - (var_num_lower) 
