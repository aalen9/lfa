...
let _dynamic_dispatch payload_field checker =
...
(** For checkers that read two separate payloads. Assumes that [checker] produces payloads for
    [payload_field1] *)
let _interprocedural2 payload_field1 payload_field2 checker =
  Procedure
...
let _interprocedural3 payload_field1 payload_field2 payload_field3 ~set_payload checker =
  Procedure
    (CallbackOfChecker.interprocedural
       ~f_analyze_dep:(fun proc_desc payloads -> Some (proc_desc, payloads))
       ~get_payload:(fun payloads ->
...
let _file issue_dir payload_field checker =
  File {callback= CallbackOfChecker.interprocedural_file payload_field checker; issue_dir}


let _intraprocedural checker = Procedure (CallbackOfChecker.intraprocedural checker)

let _intraprocedural_with_payload payload_field checker =
  Procedure (CallbackOfChecker.intraprocedural_with_field payload_field checker)


let _intraprocedural_with_field_dependency payload_field checker =
